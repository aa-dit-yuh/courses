...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        def get_ancestor_list(node):
            ancestor_list = []
            while node is not None:
                ancestor_list.insert(0, node)
                node = node.get_parent()
            return ancestor_list


        def common_length(list_a, list_b):
            if list_a[0] == list_b[0]:
                return 1 + common_length(list_a[1:], list_b[1:])
            else:
                return 0
        ...
        ancestor_a = get_ancestor_list(node_a)
        ancestor_b = get_ancestor_list(node_b)
        
        if node_a in ancestor_b or node_b in ancestor_a:
            return (-1, abs(len(ancestor_a) - len(ancestor_b)))

        common = common_length(ancestor_a, ancestor_b)
        len_a = len(ancestor_a) - common
        len_b = len(ancestor_b) - common

        degree = abs(len_a - len_b)
        cousin_type = min(len_a, len_b) - 1
...
t, r = f.cousin("a", "g")
print "'a' is a", words[t], "cousin", r, "removed from 'g'"
