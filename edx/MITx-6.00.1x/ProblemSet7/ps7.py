...
class NewsStory(object):
	...
	def __str__(self):
		return ''.join((str(self.title), str(self.subject), str(self.summary), str(self.link)))
...
class WordTrigger(Trigger):
	...
	def isWordIn(self, line):
		wordList = re.sub("[^\w]", " ",  line).split()
		for words in wordList:
			if self.word == words.lower():
				return True
		return False
...
class PhraseTrigger(Trigger):
	...
	def evaluate(self, story):
		return self.phr in story.getSubject() or self.phr in story.getTitle() or self.phr in story.getSummary()
    ...
    for trigger in triggerlist:
    	for story in stories:
    		if trigger.evaluate(story):
    			filter_stories.add(story)
    # for story in stories:
    # 	print story
    return list(filter_stories)
